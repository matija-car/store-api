...
export default function Navbar() {
    ...
    return (
        ...
        <div className="bg-burgundy px-4 py-2 text-center text-xs font-medium tracking-wide text-white">
            Informativne cijene <span className="mx-2 text-gold">•</span> Dogovor o cijeni, preuzimanju i isporuci putem e-pošte
        </div>
        <nav className="sticky top-0 z-40 border-b border-amber-900/10 bg-ivory/95 px-4 backdrop-blur sm:px-8">
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 py-4">
                ...
                <div className="hidden items-center gap-7 text-sm font-semibold text-stone-600 md:flex">
                    ...
                    <a href="/galerija#collection" className="hover:text-burgundy">Galerija</a>
                    ...
                    {user?.role === 'ADMIN' && <Link to="/admin/commissions" className="hover:text-burgundy">Zahtjevi za slike</Link>}
                </div>
                ...
                <div className="hidden items-center gap-3 md:flex">
                    ...
                    <Link to="/odabrano" className="flex items-center gap-2 rounded-lg border border-amber-900/15 px-3 py-2 text-sm font-semibold text-stone-700 hover:border-gold">
                        <span>Odabrana djela</span><span className="grid h-5 min-w-5 place-items-center rounded-full bg-gold px-1 text-[0.65rem] text-white">{cart.length}</span>
                    </Link>
                </div>
            </div>
            <div className={`${open ? 'flex' : 'hidden'} flex-col gap-2 border-t border-amber-900/10 py-4 md:hidden`}>
                ...
                <a href="/galerija#collection" onClick={() => setOpen(false)} className="px-2 py-2 text-sm font-semibold">Galerija</a>
