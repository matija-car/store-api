    ...
    return (
        <main>
            <section className="border-b border-amber-900/10 bg-cream px-4 py-10 sm:px-8">
                <div className="mx-auto max-w-7xl"><p className="eyebrow mb-3">Dom Svjetla</p><h1 className="display-font text-4xl font-bold text-ink sm:text-5xl">Darovi vjere za svaki dom.</h1><p className="mt-3 max-w-xl text-stone-500">Ikone, krunice, svijeće i pobožni predmeti odabrani s pažnjom i poštovanjem.</p></div>
            </section>
            <section id="collection" className="mx-auto max-w-7xl px-4 py-8 sm:px-8 lg:py-12">
                ...
                {products.length === 0 ? <div className="rounded-xl border border-dashed border-stone-300 py-20 text-center text-stone-500">Nema proizvoda koji odgovaraju filterima.</div> : <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div>}
